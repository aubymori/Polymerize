...
final class Loader
{
    ...
    private static function loadCulture(LanguageRecord $data): CultureInfo
    {
        ...
        if ($data->tryGetStringProperty("abbreviateNumbers", $_))
            $result->abbreviateNumbers = $_;

        if ($data->tryGetStringProperty("abbreviatedDigitCount", $_))
            $result->abbreviatedDigitCount = $_;

        if ($data->tryGetProperty("numberAbbreviations", $_))
            $result->numberAbbreviations = (array)$_;
        $_ = "";
