...
class LanguageApi
{
    ...
    public static function tryAbbreviateNumberForLanguage(
        string $langId,
        int $number,
        ?string &$out
    )
    {
        if (Loader::tryGetCulture($langId, $culture))
        {
            ...
            $bestDivisor = 0;
            $bestFormat = null;
            foreach ($culture->numberAbbreviations as $min => $format)
            {
            ...
            // ugly ugly hack to get decimal behavior the same as innertube
            $displayNumber = $number / $bestDivisor;
            $beforeDecimalLength = strlen((string)(int)$displayNumber);
            if ($beforeDecimalLength == strlen((string)(float)number_format($displayNumber, $culture->abbreviatedDigitCount - $beforeDecimalLength)))
                $decimals = 0;
            else
                $decimals = max(0, $culture->abbreviatedDigitCount - $beforeDecimalLength);
            ...
            $out = sprintf($bestFormat, $formatted);
            return true;
        }
        else
        {
            $out = number_format($number);
            return true;
        }
    }

    public static function tryAbbreviateNumber(
            int $number,
            ?string &$out
    ): bool
    {
        ...
        foreach ($langs as $langId)
        {
            if (self::tryAbbreviateNumberForLanguage($langId, $number, $result))
            {
                $out = $result;
                return true;
            }
    ...
    public static function abbreviateNumberForLanguage(
            string $langId,
            int $number
    ): string
    {
        ...
        if (self::shouldExcept())
            throw new Exception("Failed to abbreviate number.");

        return "<lang.$langId.number : $number>";
    }

    public static function abbreviateNumber(int $number): string
    {
        if (self::tryAbbreviateNumber($number, $result))
        {
            return $result;
        }
